...
class Portfolio():
    ...
    def addCash(self, amount):
        ...
        self.cash += round(float(amount), 2)
    ...
    def addProduct(self, share, name, price, producttype):
        ...
        d = {name : [round(float(share), 2), round(float(price), 2)]}
    ...
    def buyStock(self, share, obj):
        if (share - int(share)) != 0:
            raise ValueError('Only whole units can be bought')
        name = obj.name
    ...
    def sellStock(self, share, name):
        if (share - int(share)) != 0:
            raise ValueError('Only whole units can be sold')
